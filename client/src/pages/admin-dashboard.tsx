...
function TvSetupManagement() {
  ...
  const { data: tvSetupBookings = [], isLoading: loadingTvBookings } = useQuery<TvSetupBooking[]>({
    queryKey: ['/api/tv-setup/bookings'],
  });

  const deleteTvBookingMutation = useMutation({
    mutationFn: async (id: number) => {
      await apiRequest('DELETE', `/api/tv-setup/bookings/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/tv-setup/bookings'] });
      toast({
        title: "Success",
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to delete TV setup booking",
        variant: "destructive",
      });
    },
  });

  const updateTvBookingStatusMutation = useMutation({
    mutationFn: async ({ id, setupStatus, adminNotes, assignedTo, setupMethod }: { 
      id: number; 
      setupStatus: string; 
      adminNotes?: string; 
      assignedTo?: string; 
      ...
      await apiRequest('PATCH', `/api/tv-setup/bookings/${id}`, { 
        setupStatus, 
        adminNotes, 
        assignedTo, 
        setupMethod,
        completedAt: setupStatus === 'completed' ? new Date().toISOString() : null
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/tv-setup/bookings'] });
      toast({
        title: "Success",
        description: "TV setup booking updated successfully",
      });
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to update TV setup booking",
        variant: "destructive",
      });
    },
  });
  ...
  const handleEditBooking = (booking: TvSetupBooking) => {
    setSelectedBooking(booking);
    setEditForm({
      setupStatus: booking.setupStatus || 'pending',
      adminNotes: booking.adminNotes || '',
      assignedTo: booking.assignedTo || '',
      setupMethod: booking.setupMethod || ''
    });
    setIsEditDialogOpen(true);
  };
  ...
  const handleSaveEdit = () => {
    ...
    updateTvBookingStatusMutation.mutate({
      id: selectedBooking.id,
      setupStatus: editForm.setupStatus,
      adminNotes: editForm.adminNotes,
      assignedTo: editForm.assignedTo,
      setupMethod: editForm.setupMethod
    });
    setIsEditDialogOpen(false);
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>TV Setup Service Bookings</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loadingTvBookings ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2">Loading TV setup bookings...</span>
            </div>
                    ...
                    <TableRow key={booking.id}>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-gray-500">OS: {booking.tvOs}</div>
                          <div className="text-sm text-gray-500">Year: {booking.yearOfPurchase}</div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">
                          {Array.isArray(booking.streamingApps) ? 
                            booking.streamingApps.join(', ') : 
                            booking.streamingApps
                          }
                        </div>
                      </TableCell>
                      <TableCell>
                        {booking.preferredSetupDate ? 
                          new Date(booking.preferredSetupDate).toLocaleDateString() : 
                          'Not specified'
                        }
                      </TableCell>
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-gray-500 mt-1">€{booking.paymentAmount}</div>
                        </div>
                      </TableCell>
                      <TableCell>{getStatusBadge(booking.setupStatus)}</TableCell>
                      <TableCell>
                        <div className="text-sm">
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              if (confirm('Are you sure you want to delete this TV setup booking?')) {
                                deleteTvBookingMutation.mutate(booking.id);
                              }
                            }}
                            className="text-red-600 hover:text-red-700"
                          >
      ...
      {/* Edit TV Booking Dialog */}
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit TV Setup Booking</DialogTitle>
            <DialogDescription>
              Update the status and details for this TV setup booking.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label htmlFor="setupStatus">Setup Status</Label>
              <Select 
                value={editForm.setupStatus} 
                onValueChange={(value) => setEditForm({...editForm, setupStatus: value})}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select status" />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select 
                value={editForm.setupMethod} 
                onValueChange={(value) => setEditForm({...editForm, setupMethod: value})}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select method" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="remote">Remote Assistance</SelectItem>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={updateTvBookingStatusMutation.isPending}
            >
              {updateTvBookingStatusMutation.isPending ? (
                <>
                  <div className="w-4 h-4 animate-spin border-4 border-gray-300 border-t-primary rounded-full mr-2" />
                  Saving...
                </>
              ) : (
                'Save Changes'
